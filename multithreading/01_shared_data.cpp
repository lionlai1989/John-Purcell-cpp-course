...
// g++ -Wall -std=c++17 -pthread 01_shared_data.cpp && ./a.out
int main() {
    ...
    t1.join();
    std::cout << count1 << std::endl; // 100000
    ...
    t2.join();
    t3.join();
    // The output is not necessarily 200000. Usually it's smaller than 200000.
    ...
    // data.
    std::thread t4([&count3]() {
        for (int i = 0; i < ITERATIONS; i++) {
            ++count3;
        }
    });
    std::thread t5([&count3]() {
        for (int i = 0; i < ITERATIONS; i++) {
            ++count3;
        }
    });
    t4.join();
    t5.join();
    std::cout << count3 << std::endl; // The output should always be 200000.
